...
public class DebugCrosshairHook {
    ...
    public static void renderDirections(Minecraft minecraft) {
        ...
        GlStateManager.rotatef(camera.lastPitch + (camera.pitch - camera.lastPitch), -1.0F, 0.0F, 0.0F);
        GlStateManager.rotatef(camera.lastYaw + (camera.yaw - camera.lastYaw), 0.0F, 1.0F, 0.0F);
        GlStateManager.scalef(-1.0F, -1.0F, -1.0F);
        GlStateManager.scaled(1 / scale, 1 / scale, 1 / scale);
        GlStateManager.disableTexture();
        GlStateManager.depthMask(false);

        GL11.glLineWidth(minecraft.height / 540.0F);
